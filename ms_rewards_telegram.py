...
def get_search_terms():
    ...
    for date in dates:
        try:
            ...
            telegram_send.send(messages=['Error retrieving google trends json.'])
        except KeyError:
    ...
    try:
        ...
        for dict_key in login_dict_keys:
            ...
            if parser.pc_mode or parser.quiz_mode or parser.email_mode:
                ...
                telegram_send.send(messages=['Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.111 Safari/537.36 OPR/72.0.3815.186'])
                try:
                    log_in(email, password)
                    browser.get(DASHBOARD_URL)
                    if parser.pc_mode:
